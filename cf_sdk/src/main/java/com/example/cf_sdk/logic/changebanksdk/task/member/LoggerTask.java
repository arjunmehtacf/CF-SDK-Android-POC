...
public class LoggerTask extends SingleUseCase<LogParameters, ChangebankResponse> {
    ...
    @Override
    protected Single<ChangebankResponse> buildUseCaseObservable(final LogParameters logParameters) {
        return mAuthenticationDatasource.getSession()
                .flatMap(new Function<Session, SingleSource<? extends ChangebankResponse>>() {
                    ...
                    public SingleSource<? extends ChangebankResponse> apply(Session session) throws Exception {
                        LogParameters updatedLogParameters = logParameters
                                .withMemberId(session.getMemberId())
                                .withSessionId(session.getToken());
                        updatedLogParameters.addToken(session.getToken());
                        return mMemberRepository.log(updatedLogParameters);
                    }
                });
    }
}
