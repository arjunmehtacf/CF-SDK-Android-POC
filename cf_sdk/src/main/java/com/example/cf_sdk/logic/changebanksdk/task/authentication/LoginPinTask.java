...
/**
 *
 * Use case for user login with pin scenarios.
 */

public class LoginPinTask extends SingleUseCase<LoginParameters, Session> {

    private final LoginValidator mLoginValidator;
    private AuthenticationDatasource mAuthenticationRepository;

    @Inject
    public LoginPinTask(LoginValidator loginValidator,
    ...
    protected Single<Session> buildUseCaseObservable(final LoginParameters parameters) {
        return mAuthenticationRepository.getSavedUsername()
                .flatMap(new Function<Optional<String>, SingleSource<? extends Session>>() {
                    @Override
                    public SingleSource<? extends Session> apply(Optional<String> username) throws Exception {
                        parameters.setUsername(username.orNull());
