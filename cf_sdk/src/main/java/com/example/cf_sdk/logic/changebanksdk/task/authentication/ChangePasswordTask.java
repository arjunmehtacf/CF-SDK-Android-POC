...
/**
 *
 * Use case for User update password.
 */

public class ChangePasswordTask extends SingleUseCase<UpdatePasswordParameters, ChangebankResponse> {
    ...
    @Override
    protected Single<ChangebankResponse> buildUseCaseObservable(final UpdatePasswordParameters updatePasswordParameters) {
        return mAuthenticationRepository.getSession()
                .flatMap(new Function<Session, SingleSource<? extends ChangebankResponse>>() {
                    ...
                    public SingleSource<? extends ChangebankResponse> apply(Session session) throws Exception {
                        updatePasswordParameters.addToken(session.getTokenType()+" "+session.getToken());
                        updatePasswordParameters.setMemberId(session.getUsername());
                        return mAuthenticationRepository.changePassword(updatePasswordParameters);
                    }
                });
    }
}
