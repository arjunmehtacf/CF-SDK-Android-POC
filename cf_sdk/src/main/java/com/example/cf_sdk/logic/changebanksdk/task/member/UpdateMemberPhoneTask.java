...
/**
 *
 * Use case for updating member Phone.
 */

public class UpdateMemberPhoneTask extends SingleUseCase<VerifyPhoneCodeParameters, ChangebankResponse> {

    private AuthenticationDatasource mAuthenticationDatasource;
    private MemberDatasource mMemberRepository;

    @Inject
    public UpdateMemberPhoneTask(
    ...
    @Override
    protected Single<ChangebankResponse> buildUseCaseObservable(final VerifyPhoneCodeParameters verifyPhoneCodeParameters) {
        return mAuthenticationDatasource.getSession()
                .flatMap(new Function<Session, SingleSource<? extends ChangebankResponse>>() {
                    ...
                    public SingleSource<? extends ChangebankResponse> apply(Session session) throws Exception {
                        verifyPhoneCodeParameters.addToken(session.getTokenType()+" "+session.getToken());
                        return mMemberRepository.updateMemberPhone(verifyPhoneCodeParameters);
                    }
                });
    }
}
